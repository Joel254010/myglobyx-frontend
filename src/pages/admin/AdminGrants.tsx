...
import { listProducts, listGrants, grantAccess, revokeAccess, AdminProduct, Grant } from "../../lib/adminApi";
...
export default function AdminGrants() {
  ...
  async function onGrant(e: React.FormEvent) {
    e.preventDefault();
    setMsg(null);
    if (!email || !productId) { setMsg("Informe e-mail e produto"); return; }
  ...
  return (
    <div>
      ...
      <form onSubmit={onGrant} className="card" style={{ padding: 12, marginBottom: 16 }}>
        ...
        <div className="actions"><button className="btn btn--primary">Conceder</button></div>
      </form>

      <div className="card" style={{ padding: 12 }}>
        <h3>Liberações {email ? `de ${email}` : ""}</h3>
        <div className="row" style={{ gap: 8, marginBottom: 8 }}>
          <input className="input" placeholder="Filtrar por e-mail" value={email} onChange={(e) => setEmail(e.target.value)} />
          <button className="btn btn--ghost" onClick={() => refresh(email || undefined)}>Aplicar filtro</button>
          <button className="btn btn--outline" onClick={() => { setEmail(""); refresh(); }}>Limpar</button>
        </div>
        {grants.length === 0 ? (
              ...
              return (
                <li key={g.id} className="list__row">
                  <div>
                    ...
                    <div className="muted small">{new Date(g.createdAt).toLocaleString()}</div>
                  </div>
                  <button className="btn btn--outline" onClick={() => onRevoke(g)}>Revogar</button>
                </li>
              );
            })}
          </ul>
